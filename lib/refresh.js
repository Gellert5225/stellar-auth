...
exports.signOut = async (session_id, db) => {
    return await db.database.collection('Session').deleteOne({ _id: db.mongodb.ObjectID(session_id) });
}
...
exports.refreshJWT = async (session_id, db, jwtSecret) => {
    return new Promise((resolve, reject) => {
        (async () => {
            try {
                const session = await db.database.collection('Session').findOne({ _id: db.mongodb.ObjectID(session_id) });
                if (!session) 
                ...
                try {
                    await this.signOut(session_id, db);
                } catch (error) {
